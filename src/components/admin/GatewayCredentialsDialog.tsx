...
export function GatewayCredentialsDialog({
  ...
  const fetchCredentials = async () => {
    ...
    try {
      ...
      console.error('[GatewayCredentials] Error:', err);
  ...
  const saveCredentials = async () => {
    ...
    try {
      if (isSuperAdmin) {
        // Super admin: save via set-gateway-credentials edge function
        const { data, error: invokeError } = await supabase.functions.invoke('set-gateway-credentials', {
          body: {
            gateway,
            credentials: {
        ...
        // Regular admin: save to seller_gateway_credentials
        ...
        if (gatewayError || !gatewayData) throw new Error('Gateway não encontrado');
        ...
        if (existing) {
          const { error: updateError } = await supabase
            .from('seller_gateway_credentials')
            .update({ credentials: credentialData, is_active: true, updated_at: new Date().toISOString() })
            .eq('id', existing.id);
          if (updateError) throw updateError;
        } else {
          const { error: insertError } = await supabase
            .from('seller_gateway_credentials')
            .insert({ user_id: user.id, gateway_id: gatewayData.id, credentials: credentialData, is_active: true });
  ...
  const copyToClipboard = async (key: 'client_id' | 'client_secret', label: string) => {
    ...
    if (!value) { toast.error('Valor não disponível para copiar'); return; }
    try {
  ...
  const getMaskedValue = (value: string | null) => !value ? 'Não configurado' : '••••••••••••••••••••••';
  ...
  const fields = gateway === 'ghostpay' 
    ...
    : gateway === 'sigilopay'
    ? [
        { key: 'client_id' as const, label: 'Chave Pública (Client ID)' },
        { key: 'client_secret' as const, label: 'Chave Privada (Client Secret)' },
      ]
    : [
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {isSuperAdmin 
              ? `Gerencie as credenciais globais do gateway ${gatewayName}`
          ...
          {!loading && !error && !isEditing && (
            <div className={`p-3 rounded-lg text-sm ${isSuperAdmin ? 'bg-blue-500/10 border border-blue-500/20' : 'bg-yellow-500/10 border border-yellow-500/20'}`}>
              <p className={isSuperAdmin ? 'text-blue-600 dark:text-blue-400' : 'text-yellow-600 dark:text-yellow-400'}>
                {isSuperAdmin ? (
                  <>
                    <strong>Super Admin:</strong> Estas são as credenciais globais da plataforma. Clique em "Editar" para configurá-las.
                  </>
                ) : (
                  <>
