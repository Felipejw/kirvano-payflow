...
const differentials = [{
  icon: Ban,
  text: "0 Estorno - PIX não tem chargeback"
}, {
  icon: Wallet,
  text: "Sem Taxa de Saque - Receba 100%"
}, {
  icon: Clock,
  text: "Saque 24/7 - A qualquer hora"
}];
export function Pricing() {
  ...
  return <section id="pricing" className="py-24 relative overflow-hidden">
      ...
      <div className="absolute bottom-1/4 left-0 w-96 h-96 bg-accent/5 rounded-full blur-3xl animate-float" style={{
      animationDelay: '2s'
    }} />
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div ref={cardRef} className={`max-w-xl mx-auto transition-all duration-700 ${cardVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-12 scale-95'}`} style={{
        transitionDelay: '200ms'
      }}>
          <Card variant="glass" className="relative border-primary/50 shadow-glow-primary hover:shadow-glow-accent transition-all duration-500 group">
            ...
            <CardContent className="space-y-8">
              ...
              <div className="space-y-3">
                ...
                {differentials.map((diff, index) => <div key={diff.text} className={`flex items-center gap-3 p-3 rounded-lg bg-secondary/30 hover:bg-secondary/50 hover:scale-[1.02] transition-all duration-300 cursor-default group/item ${cardVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'}`} style={{
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {features.map((feature, index) => <div key={feature} className={`flex items-start gap-2 hover:translate-x-1 transition-all duration-300 cursor-default ${cardVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`} style={{
